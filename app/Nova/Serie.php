...
class Serie extends Resource
{
    ...
    public function fields(NovaRequest $request)
    {
        return [
            ...
            Hidden::make('Show Type ID', 'show_type_id')->default(2),
            // genres test 1
            ...
            ->maxWidth(150)
            ->disk('public')
            ->path('img/series_cover_collection')
            ->storeAs(function (Request $request){
                ...
                if(Storage::disk('public')->exists('img/series_cover_collection/'.$newFileName)){
            ...
            ->maxWidth(150)
            ->disk('public')
            ->path('img/series-banner-collection')
            ->storeAs(function (Request $request){
                $originalFileName = $request->banner_path->getClientOriginalName();
                //$newFileName = str_replace(' ', '-', $originalFileName);
                $newFileName = $this->slug . "-" . $this->id . "-" . str_replace(' ', '-', strtolower($originalFileName));
                if(Storage::disk('public')->exists('img/series-banner-collection/'.$newFileName)){
                    $newFileName = $this->slug . "-" . $this->id . "-2-" . str_replace(' ', '-', strtolower($originalFileName));
                }
                return $newFileName;
            })
            ...
            // instalments
            HasMany::make('Instalments', 'instalments'),
            // accolades
    ...
    {
        return $query->where('show_type_id', 2);
    }
    ...
    {
        return [
            (new Actions\ImportSeries())->showInline()->standalone(),

            (new Actions\PublishShows())->showInline()
            ->confirmText('Are you sure you want to publish this serie?')
            ->confirmButtonText('Publish')
            ->cancelButtonText("Cancel"),
            
            (new Actions\UnpublishShows())->showInline()
            ->confirmText('Are you sure you want to unpublish this serie?')
            ->confirmButtonText('Unpublish')
            ->cancelButtonText("Cancel"),

            (new Actions\ExportSeries())->showInline()
        ];
    }
