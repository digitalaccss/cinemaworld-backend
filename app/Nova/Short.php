...
class Short extends Resource
{
    ...
    public function fields(NovaRequest $request)
    {
        return [
            ...
            // slug
            Slug::make('Slug', 'slug')->from('Title')->onlyOnForms()->rules('required', 'max:60'),
            // show type
            // BelongsTo::make('Show Type', 'showType'),
            Hidden::make('Show Type ID', 'show_type_id')->default(4),
            // genres test 1
            ...
            ->maxWidth(150)
            ->disk('public')
            ->path('img/short-cover-collection')
            ->storeAs(function (Request $request){
                ...
                if(Storage::disk('public')->exists('img/short-cover-collection/'.$newFileName)){
            ...
            ->maxWidth(150)
            ->disk('public')
            ->path('img/short-banner-collection')
            ->storeAs(function (Request $request){
                $originalFileName = $request->banner_path->getClientOriginalName();
                //$newFileName = str_replace(' ', '-', $originalFileName);
                $newFileName = $this->slug . "-" . $this->id . "-" . str_replace(' ', '-', strtolower($originalFileName));
                if(Storage::disk('public')->exists('img/short-banner-collection/'.$newFileName)){
                    $newFileName = $this->slug . "-" . $this->id . "-2-" . str_replace(' ', '-', strtolower($originalFileName));
                }
                return $newFileName;
            })
            ...
            // ->maxWidth(150)
            // ->disk('public')
            // ->path('img/short-leaderboard-banner-collection')
            // ->storeAs(function (Request $request){
            ...
            ->maxWidth(150)
            ->disk('public')
            ->path('img/short-tonight-banner-web-collection')
            ->storeAs(function (Request $request){
                $originalFileName = $request->web_tonight_banner_photo_path->getClientOriginalName();
                //$newFileName = str_replace(' ', '-', $originalFileName);
                $newFileName = $this->slug . "-" . $this->id . "-" . str_replace(' ', '-', strtolower($originalFileName));
                if(Storage::disk('public')->exists('img/short-tonight-banner-web-collection/'.$newFileName)){
                    $newFileName = $this->slug . "-" . $this->id . "-2-" . str_replace(' ', '-', strtolower($originalFileName));
                }
                return $newFileName;
            })
            ...
            ->maxWidth(150)
            ->disk('public')
            ->path('img/short-tonight-banner-mobile-collection')
            ->storeAs(function (Request $request){
                $originalFileName = $request->mobile_tonight_banner_photo_path->getClientOriginalName();
                //$newFileName = str_replace(' ', '-', $originalFileName);
                $newFileName = $this->slug . "-" . $this->id . "-" . str_replace(' ', '-', strtolower($originalFileName));
                if(Storage::disk('public')->exists('img/short-tonight-banner-mobile-collection/'.$newFileName)){
                    $newFileName = $this->slug . "-" . $this->id . "-2-" . str_replace(' ', '-', strtolower($originalFileName));
                }
                return $newFileName;
            })
    ...
    {
        return $query->where('show_type_id', 4);
    }
    ...
    public function actions(NovaRequest $request)
    {
        return [
            ...
            ->confirmText('Are you sure you want to publish this short?')
            ->confirmButtonText('Publish')
            ->cancelButtonText("Cancel"),
            
            (new Actions\UnpublishShows())->showInline()
            ->confirmText('Are you sure you want to unpublish this short?')
            ->confirmButtonText('Unpublish')
            ->cancelButtonText("Cancel"),

            //(new DownloadExcel)
            (new Actions\ExportShows())->showInline()
            // (new DownloadExcel)->only('id', 'house_media_id', 'title', 'foreign_title', 'slug', 'release_year', 'runtime', 'short_desc', 'full_desc', 'trivia_desc', 'trailer_link_url', 'director_statement', Date('expiry_date')->format('Y-m-d'))
            // ->withHeadings('id', 'house media id', 'title', 'foreign title', 'slug', 'release year', 'runtime', 'short desription', 'full description', 'trivia description', 'trailer link', 'director statement', 'expiry date')
