...
{
    public function getAllShorts(Request $req){
        $showModel = (new Show)->newQuery();
        $perpage = $req->perpage;

        if($req->has('countries')){
            $countryArr = json_decode($req->countries);
            $countryIDArr = Country::whereIn('country_name', $countryArr)->pluck('id');

            $showIDs = ShowCountry::whereIn('country_id', $countryIDArr)->distinct()->pluck('show_id');
        ...
        if(count($shorts) >= 1){
            ...
            foreach($shorts as $indexPos => $short){
                $data[$indexPos]['id'] = $short->id;
                $data[$indexPos]['title'] = $short->title;
                $data[$indexPos]['foreign_title'] = $short->foreign_title;
                $data[$indexPos]['sub_title'] = $short->sub_title;
                $data[$indexPos]['expiry_date'] = ($short->expiry_date == null)? null: $short->expiry_date->format('Y-m-d');
                $data[$indexPos]['slug'] = '/shorts/'.$short->slug;
                ...
                if($short->cover_photo_path){
                    if(Storage::disk('public')->exists($short->cover_photo_path)){
                        $data[$indexPos]['cover_photo_path'] = Storage::url($short->cover_photo_path);
                    }
                    else {
            ...
            return response()->json('No shorts were found that matches your search', 202);
        }
    }
    ...
    public function getShort(Request $req){
        ...
        if($short){
            ...
            $data['foreign_title'] = $short->foreign_title;
            $data['sub_title'] = $short->sub_title;
            $data['expiry_date'] = ($short->expiry_date == null)? null: $short->expiry_date->format('Y-m-d');
            // $data['slug'] = '/shorts/'.$short->slug;
            $data['show_type'] = ShowType::where('id', $short->show_type_id)->value('show_type');
            $data['release_year'] = $short->release_year;
            $data['runtime'] = $short->runtime;
            $data['short_desc'] = $short->short_desc;
            $data['full_desc'] = $short->full_desc;
            $data['trivia_desc'] = $short->trivia_desc;
            $data['trailer_link_url'] = $short->trailer_link_url;
            ...
            $countryIDs = ShowCountry::where('show_id', $short->id)->pluck('country_id');
            if(count($countryIDs) >= 1){
                $countries = Country::whereIn('id', $countryIDs)->pluck('country_display_name');
                $data['countries'] = $countries;
            }

            $languageIDs = Showlanguage::where('show_id', $short->id)->pluck('language_id');
            ...
            $critics = Critic::select('name', 'critique')->where('show_id', $short->id)->get();
            if(count($critics) >= 1){
            ...
            if($short->banner_path){
                if(Storage::disk('public')->exists($short->banner_path)){
                    $data['banner_path'] = Storage::url($short->banner_path);
                }
                else {
            ...
            if($short->media){
                ...
                foreach($short->getMedia('short-gallery-collection') as $photoGalleryObj){
            ...
            if($short->director_photo_path){
                if(Storage::disk('public')->exists($short->director_photo_path)){
                    $data['director_photo_path'] = Storage::url($short->director_photo_path);
                }
                else {
                ...
                if(count($showsWithSameGenres) >= 1){
                    ...
                    if(count($youMayAlsoLikes) >= 1){
                        foreach($youMayAlsoLikes as $indexPos => $youMayAlsoLike){
                            $data['you_may_also_likes'][$indexPos]['short_id'] = $youMayAlsoLike->id;
                            ...
                            $data['you_may_also_likes'][$indexPos]['slug'] = '/shorts/'.$youMayAlsoLike->slug;
            ...
            $nowPlayingShort = Schedule::select('id', 'show_id')
            ...
            ->where('show_id', $short->id)
            ->first();

            $scheduleModel = (new Schedule)->newQuery();

            // if($nowPlayingShort){
            //     $scheduleModel->where('id', '!=', $nowPlayingShort->id)->where('show_id', $nowPlayingShort->show_id);
            // }
            // else {
            //     $scheduleModel->where('show_id', $short->id);
            // }
            ...
            return response()->json('This short was not found', 202);
        }
    }
}
