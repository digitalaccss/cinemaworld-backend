...
class SearchController extends Controller
{
    public function search(Request $req){
        ...
        if($searchStr){
            ...
            $cacheKey = 'search_' . md5(strtolower(trim($searchStr)));
            
            return Cache::remember($cacheKey, 300, function () use ($searchStr) {
                return $this->performSearch($searchStr);
            });
        }
        else {
    ...
    private function performSearch($searchStr) {
        $results = [];
